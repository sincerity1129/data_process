...
def pdf_search_label_location_count(self, preprocess_table):
    ...
    for label in preprocess_table[1:]:
        if label[0] == None:
            ...
            break
    _pdf_table_convert(preprocess_table, label_count)
    return label_count

@shared_task
def pdf_table_label_parsing(label_count, table_labels):
    label_list = []
    if label_count == 0:
...
@shared_task
def pdf_table_result_label_list(label_list):
...
@shared_task
def pdf_table_result_list(label_count, label_list, preprocess_table):
    result_list = []
...
@shared_task
def result_cleaning(result_dict):
    import os
    from controllers.common_controller import json_dump, json_load
...
def total_pdf_parsing(
    ...
    from controllers.common_controller import txt_dump
